...
import System.IO (hFlush, hSeek, hGetContents, hPutStr, SeekMode(AbsoluteSeek))
...
import Text.Megaparsec (ParseError, PosState, ParseErrorBundle)
...
|]
  where
    displayField field = E.getFieldKey (fiName field) <> " = " <> displayFieldContents (fiContents field)
